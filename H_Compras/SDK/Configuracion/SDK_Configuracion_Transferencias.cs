...
{
    public class SDK_Configuracion_Transferencias
    {
        decimal IdDocumento;
        SDK.Documentos.frmTransferencia Form_Traspaso;
        ...
        public void Header()
        {
            using (SqlConnection connection = new SqlConnection(Datos.Clases.Constantes.conectionLog))
            {
                using (SqlCommand command = new SqlCommand("sp_SDKDocuments", connection))
                {
                    ...
                    foreach (Control item in Form_Traspaso.Controls)
                    {
        ...
        public void Lines()
        {
            ...
            using (SqlConnection connection = new SqlConnection(Datos.Clases.Constantes.conectionLog))
            {
                using (SqlCommand command = new SqlCommand("sp_SDKDocuments", connection))
                {
                    ...
                    Form_Traspaso.dgvDatos.InitializeLayout -= new Infragistics.Win.UltraWinGrid.InitializeLayoutEventHandler(dgvDatos_InitializeLayout);
                    Form_Traspaso.dgvDatos.InitializeLayout += new Infragistics.Win.UltraWinGrid.InitializeLayoutEventHandler(dgvDatos_InitializeLayout);

                    Form_Traspaso.ultraDropDown1.InitializeLayout -= new Infragistics.Win.UltraWinGrid.InitializeLayoutEventHandler(dgvCombo_InitializeLayout);
                    Form_Traspaso.ultraDropDown1.InitializeLayout += new Infragistics.Win.UltraWinGrid.InitializeLayoutEventHandler(dgvCombo_InitializeLayout);

                    Form_Traspaso.dgvDatos.AfterCellListCloseUp -= new CellEventHandler(dgvDatos_AfterCellListCloseUp);
                    Form_Traspaso.dgvDatos.AfterCellListCloseUp += new CellEventHandler(dgvDatos_AfterCellListCloseUp);

                    Form_Traspaso.dgvDatos.AfterCellUpdate -= new Infragistics.Win.UltraWinGrid.CellEventHandler(dgvDatos_AfterCellUpdate);
                    Form_Traspaso.dgvDatos.AfterCellUpdate += new Infragistics.Win.UltraWinGrid.CellEventHandler(dgvDatos_AfterCellUpdate);
                }
            }

            tbl_Lines.Clear();
            string curr = Form_Traspaso.txtMoneda.Text == "USD" ? "USD" : string.Empty;
            ...
            Form_Traspaso.dgvDatos.DataSource = tbl_Lines;
            #endregion
        }
        ...
        public void StartEmpty()
        {
            using (SqlConnection connection = new SqlConnection(Datos.Clases.Constantes.conectionLog))
            {
                using (SqlCommand command = new SqlCommand("sp_SDKDocuments", connection))
                {
                    ...
                    //Form_Traspaso.Text = tbl.Rows[0].Field<string>("Nombre");
        ...
        string _whsCodeTransit;
        string _whsNameTransit;
        public void GetTransito(string _whsCode)
        {
            _whsNameTransit = string.Empty;
            _whsCodeTransit = string.Empty;

            var qry = (from item in tbl_Almacenes.AsEnumerable()
                       where item.Field<string>("U_VLGX_RHT") == _whsCode
                       select new
                       {
                           WhsCode = item.Field<string>("code"),
                           WhsName = item.Field<string>("name")
                       }).FirstOrDefault();

            _whsNameTransit = qry.WhsName;
            _whsCodeTransit = qry.WhsCode;
        }

        public void StartFill(int _idDocument, DataTable tblSource, string origen, string destino)
        {
            ...
            Form_Traspaso.txtToWhsCode.Text = destino;
            Form_Traspaso.txtFiller.Text = origen;
            ...
            if (IdDocumento == (int)DocumentType.Transferencia)
            {
                DataTable details = new DataTable();
                var result = from item in tblSource.AsEnumerable()
                             where item.Field<decimal>("Cantidad") > 0
                             select item;
                ...
                foreach (DataRow item in details.Rows)
                {
                    ...
                    this.GetTransito(item["WhsCode"].ToString());

                    row["WhsCode"] = _whsCodeTransit;//destino
                    row["WhsName"] = _whsCodeTransit + " | " + _whsNameTransit;//destino

                    row["FromWhsCode"] = item["FromWhsCode"];//origen
                    row["FromWhsName"] = item["FromWhsCode"] + " | " + item["FromWhsName"];//origen
            ...
            Form_Traspaso.txtComments.Text = "Transferencia generada por HalcoNET";
        }
        ...
        private void dgvDatos_AfterCellListCloseUp(object sender, CellEventArgs e)
        {
            try
            {
                if (e.Cell.Column.Index > -1)
                {
                    ...
                    if (e.Cell.Column.Key.Equals("SlpCpde") || e.Cell.Column.Key.Equals("WhsName") || e.Cell.Column.Key.Equals("FromWhsName") || e.Cell.Column.Key.Equals("U_TipoAlmName"))
                        Form_Traspaso.dgvDatos.Rows[e.Cell.Row.Index].Cells[e.Cell.Column.Index - 1].Value = e.Cell.Text.Substring(0, e.Cell.Text.IndexOf('|'));
                   
                    if (e.Cell.Column.Key.Equals("ItemCode"))
                    {
                        Form_Traspaso.dgvDatos.Rows[e.Cell.Row.Index].Cells["ItemName"].Value = Form_Traspaso.ultraDropDown1.SelectedRow.Cells["ItemName"].Value;
                        ...
                        Form_Traspaso.dgvDatos.Rows[e.Cell.Row.Index].Cells["ManBtchNum"].Value = Form_Traspaso.ultraDropDown1.SelectedRow.Cells["ManBtchNum"].Value;
                    }
                }
            }
            catch (Exception) { }
