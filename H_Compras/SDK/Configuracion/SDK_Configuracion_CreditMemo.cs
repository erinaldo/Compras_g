...
{
    public class SDK_Configuracion_CreditMemo
    {
        decimal IdDocumento;
        SDK.Documentos.frmCreditMemo Form_Document;
        ...
        public SDK_Configuracion_CreditMemo(decimal _idDocumento, SDK.Documentos.frmCreditMemo form)
        {
            tbl_Lines = new DataTable();
            tbl_Items = new DataTable();
            tbl_Cuentas = new DataTable();
        ...
        public void Header()
        {
            using (SqlConnection connection = new SqlConnection(Datos.Clases.Constantes.conectionLog))
            {
                using (SqlCommand command = new SqlCommand("sp_SDKDocuments", connection))
                {
                    ...
                    ObjType = Convert.ToInt32(tbl.Rows[0]["ObjType"]);
                }
            }
        ...
        public void Lines()
        {
            ...
            using (SqlConnection connection = new SqlConnection(Datos.Clases.Constantes.conectionLog))
            {
                using (SqlCommand command = new SqlCommand("up_SDKDocuments", connection))
                {
                    command.CommandType = CommandType.StoredProcedure;
                    ...
                    Form_Document.udpCuentas.SetDataBinding(tbl_Cuentas, null);
                    Form_Document.udpCuentas.ValueMember = "AcctCode";
                    Form_Document.udpCuentas.DisplayMember = "AcctCode";
                }
            }
            #endregion
        }
        ...
        public void FillDetails(DataTable tblSource)
        {
            ...
            foreach (DataRow item in details.Rows)
            {
                ...
                foreach (DataRow line in tbl_Lines.Rows)
                {
                    if (line.RowState != DataRowState.Deleted)
                        if (line["BaseLine"] != DBNull.Value)
                            ...
                            {
                                Form_Document.SetMensaje("Ya se ha agregado este Documento.", 5000, Color.Red, Color.White);
                                return;
                            }
                }
                DataRow row = tbl_Lines.NewRow();
                ...
                row["Price"] = item["Price"];
                row["Currency"] = item["Currency"];
                row["Rate"] = item["Currency"].ToString().Equals(Form_Document.txtMoneda.Text) ? 1 : _rate;
                            ...
                            if (row.Field<bool>("listColumn"))
                            {
                                ...
                                using (SqlConnection connection = new SqlConnection(Datos.Clases.Constantes.conectionLog))
                                {
                                    using (SqlCommand command = new SqlCommand("sp_SDKDataSource", connection))
                                    {
                                        ...
                                        else if (row.Field<string>("nameColumn").Equals("AccountCode"))
                                        {
                                            item.ValueList = this.Form_Document.udpCuentas;
                                        }
                                        else
                                        {
                                            ValueList vl;
                ...
                if (e.Cell.Column.Index > -1)
                {
                    ...
                    if (e.Cell.Column.Key.Equals("ItemCode"))
                    {
                        ...
                        Form_Document.dgvDatos.Rows[e.Cell.Row.Index].Cells["Rate"].Value =
                            Form_Document.txtMoneda.Text.Equals(Form_Document.ultraDropDown1.SelectedRow.Cells["Currency"].Value) ? 1 :
                            Convert.ToDecimal(Form_Document.txtTC.Text);
                        ...
                        Form_Document.dgvDatos.Rows[e.Cell.Row.Index].Cells["TaxCode"].Value = Form_Document.txtTaxCode.Text;
                    }
                }
            }
            catch (Exception) { }
