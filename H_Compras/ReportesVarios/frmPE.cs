...
namespace H_Compras.ReportesVarios
{
    public partial class frmPE : Form
    {
        ...
        public void Formato(DataGridView dgv)
        {
            ...
            dgv.Columns[(int)Columnas.PE].Width = 40;
            ...
            if ((int)ClasesSGUV.Login.Rol == (int)ClasesSGUV.Propiedades.RolesHalcoNET.GerenteCompras
                || (int)ClasesSGUV.Login.Rol == (int)ClasesSGUV.Propiedades.RolesHalcoNET.Zulma)
                dgv.Columns[(int)Columnas.PE].ReadOnly = false;
            else
                    ...
                    if (row.RowState == DataRowState.Added || row.RowState == DataRowState.Modified)
                    {
                        using (SqlConnection connection = new SqlConnection(ClasesSGUV.Propiedades.conectionSGUV))
                        {
                            using (SqlCommand command = new SqlCommand("sp_PreciosEntrega", connection))
                            {
                                ...
                                command.Parameters.AddWithValue("@EntradaSAP", row.Field<string>("Factor real de gasto"));
                                ...
                                command.Parameters.AddWithValue("@PE", row.Field<bool>("Arribo CEDIS"));
                                command.Parameters.AddWithValue("@TT_LC", row.Field<string>("TT-LC"));
            ...
            try
            {
                if (checkBox1.Checked)
                {
                    ...
                    tbl = (from item in __Datos.AsEnumerable()
                           where string.IsNullOrEmpty(item.Field<string>("Num de Precio Entrega"))
